...
class Usuario {
    ...
    public function consultarUsuariosProfesorPorId($id){
        ...
        $query = $this->db->prepare("SELECT * FROM usuario, profesor,materias where usuario.ididentidad = profesor.idprofesor and profesor.idmateria=materias.idmateria and idusuario=".$id."");
        $query->execute(array('id' => $id));
        $resultado=$query->fetchAll();
