...
class Alumno {
    ...
    public function consultarAlumnosporid($id){
        ...
        public function crearAlumno($id,$nombre,$direccion,$celular,$telefono,$sexo,$idcurso,$idjornada){

            try {
                
                $query = $this->db->prepare(' insert into alumno values( :id, :nombre, :direccion, :celular, :telefono, :sexo, :idcurso, :idjornada) ');
                $e=$query->execute(array('id' => $id,'nombre' => $nombre, 'direccion' => $direccion, 'celular' => $celular, 'telefono' => $telefono, 'sexo' => $sexo,'idcurso' => $idcurso, 'idjornada' => $idjornada));
                }
            catch(PDOException $e){
                echo $e->getMessage();
            }
            if($query){
             echo'<div class="center-aligner center  light-blue white-text btn">Guardado!!</div>'.$id.'.'.$nombre.''.$direccion.'.'.$celular.'.'.$telefono.'.'.$sexo.'.'.$idcurso.'.'.$idjornada ;
            }
        }
